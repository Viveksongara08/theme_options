...
foreach($image_data as $imagedata){
   ...
   $imagelink =get_post_meta ($imagedata, $key = '_wp_attached_file', $single = false );
	
$newarr.="<div style='height:70px; width:70px;float: left;border: solid 3px #000;margin: 10px;' id=post_".$imagedata." ><img src=".site_url()."/wp-content/uploads/".$imagelink[0]."  class='img-responsive'>
<div class='uni-upload-de-fix'><button type='button' style='cursor: pointer;'  id=".$imagedata." class='delete remove_image'> <i class='fa fa-trash-o' aria-hidden='true'></i>Remove</button></div></div>";
